...
func main() {
	f, err := os.Open("2021/04/pt2/input.txt")
	if err != nil {
		panic(err)
	}
	defer f.Close()
	...
	cardWins := make(map[int][][]cardNumber)
	var lastWinCard [][]cardNumber
	var lastWinPick int
	for _, pick := range pickedNumbers {
		fmt.Printf("%d\n\n", pick)
		for i, card := range cards {
			if _, ok := cardWins[i]; ok {
				continue
			}
	...
	fmt.Printf("score %d", score(lastWinCard, lastWinPick))
	return
}

func updateWins(cardWins []int, newCardID int) {
	for _, cardID := range cardWins {
		if cardID == newCardID {
			return
		}
	}
	cardWins = append(cardWins, newCardID)
}
