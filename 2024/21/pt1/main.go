...
func main() {
	...
	for _, code := range codes {
		...
		path := recursePaths(numpadShortestPaths, 3, code)
		fmt.Println(nCode, len(path), path)
		sum += nCode * len(path)
	}
...
func recursePaths(pad map[rune]map[rune]string, depth int, path string) string {
	if depth == 0 {
		return path
	}
	...
	for _, key := range []rune(path) {
		...
		outPath += recursePaths(dirpadShortestPaths, depth-1, shortestPath)
		currentKey = key
	}
	return outPath
}
